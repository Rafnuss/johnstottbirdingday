...
async function fetchUserData() {
  const response = await fetch(
    `https://sheets.googleapis.com/v4/spreadsheets/1GtyqPSkGOwpwHsez6kc4KWx_n2nMu45iMPf5sJHsBXc/values/A1:F100?key=${google_api_key}`
  );
...
async function fetchTripReport(timeout = 5000, callback) {
  ...
  try {
    ...
    for (let i = 0; i < user.length; i++) {
      ...
      try {
        const num_sp_res = await fetch(
          "http://tripreport.raphaelnussbaumer.com/tripreport-internal/v1/num-species/" +
            user[i].tripreport
        );
        ...
        const check_res = await fetch(
          "http://tripreport.raphaelnussbaumer.com/tripreport-internal/v1/checklists/" +
            user[i].tripreport
        );
        ...
        const tax_res = await fetch(
          "http://tripreport.raphaelnussbaumer.com/tripreport-internal/v1/taxon-list/" +
            user[i].tripreport
        );
        ...
        tax_data = tax_data
          .filter((s) => {
            return (
              (s.category == "species") &
              (!s.hasOwnProperty("exoticCategory") || s.exoticCategory != "X")
            );
          })
          .map((s) => s.speciesCode);
        ...
        console.log("Error with " + user[i].name + ":");
        console.log(error);
        error_user.push(user[i].email);
      }
    }
    console.log("\n-> fetched all data\n");

    // Filter user with error
    user = user.filter((u) => !error_user.includes(u.email));
