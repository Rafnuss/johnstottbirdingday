...
async function fetchUserData() {
    const response = await fetch(`https://sheets.googleapis.com/v4/spreadsheets/1GtyqPSkGOwpwHsez6kc4KWx_n2nMu45iMPf5sJHsBXc/values/A1:E100?key=${google_api_key}`);
    ...
    const user = rows.map((row) => {
        return {
            ...
            name: row[2],
            party: row[3],
            tripreport: row[4].replace("https://ebird.org/tripreport/", "")
        };
    });
    return user
}
